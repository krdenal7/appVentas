...
public class ChatService extends Service {
    ...
    public void onCreate() {
        ...
        WindowManager.LayoutParams params = ParametroInicial();
    ...
    private WindowManager.LayoutParams ParametroInicial(){
    ...
    private void moveToLeft(int x_cord_now){
        ...
        new CountDownTimer(500, 5) {
            ...
            WindowManager.LayoutParams bParams=(WindowManager.LayoutParams)chatheadView.getLayoutParams();

            public void onTick(long t) {
                long step = (500 - t)/5;
                mParams.x = (int)(double)bounceValue(step,x);
                bParams.x = (int)(double)bounceValue(step-20,x);
                windowManager.updateViewLayout(chatheadView, mParams);
                windowManager.updateViewLayout(txtView,bParams);
            }
    ...
    private  void moveToRight(int x_cord_now){
        ...
        new CountDownTimer(500, 5) {
            ...
            public void onTick(long t) {
                ...
                bParams.x = szWindow.x + (int)(double)bounceValue(step-20,x) - chatheadView.getWidth();
                windowManager.updateViewLayout(chatheadView, mParams);
                windowManager.updateViewLayout(txtView,bParams);
            }
            public void onFinish() {
                mParams.x = szWindow.x - chatheadView.getWidth();
                bParams.x = szWindow.x - chatheadView.getWidth();
                windowManager.updateViewLayout(chatheadView, mParams);
                windowManager.updateViewLayout(txtView,bParams);
            }
        }.start();
    ...
    private void chathead_click(){
        if(Dialog.active){
            ...
            Intent it = new Intent(this,Dialog.class).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_CLEAR_TOP);
            startActivity(it);
            WindowManager.LayoutParams params=ParametroInicial();
    ...
    private void showMsg(String sMsg,String pMsg){
        ...
        if(txtView != null && chatheadView != null ){
            ...
            if(!Dialog.active) {
                txt1.setText(sMsg);
                WindowManager.LayoutParams mParams = (WindowManager.LayoutParams) chatheadView.getLayoutParams();
                windowManager.updateViewLayout(txtView, mParams);
                txtView.setVisibility(View.VISIBLE);
            }else{

                Context ct=Dialog.myDialog.context;
                ListView ls=(ListView)((Activity)ct).findViewById(R.id.listView);
                SimpleAdapter adapter=adapter(ls.getAdapter(),ct,pMsg);
                ls.setAdapter(adapter);
                ls.setSelection(ls.getAdapter().getCount()-1);
            }
    ...
    public void EliminarPrefMensajes(){
    ...
    public SimpleAdapter adapter(Adapter adapt,Context context,String msg){
        ...
        for (int i=0;i<adapt.getCount();i++){
            Object r=adapt.getItem(i);
            row=(HashMap<String,String>)r;
            data.add(row);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if(bd != null){
            ...
            final String pMsg=bd.getString("msj_push");
            showMsg(sMsg,pMsg);
        }
